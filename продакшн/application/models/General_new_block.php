<?php
	class General_new_block extends CI_Model{
		...
		public function query_select_block(){
			$this->load->database();
			$sql = 'SELECT block_name.block_title, block_name.block_content, category_id FROM block_name';
			$query = $this->db->query($sql);
			foreach($query->result() as $row){
				$this->array['array' . $this->cnt]['block_title'] = $row->block_title; 
				$this->array['array' . $this->cnt]['block_content'] = $row->block_content;
				$this->array['array' . $this->cnt]['category_id'] = $row->category_id;
				++$this->cnt;
			}
		...
		public function select_block_adminpanel(){
			...
			$this->load->database();
			$sql = "SELECT block_name.block_title, block_name.block_content, category_id, 
		...
		public function query_insert_block($block_title, $block_content, $category_id, $login){
			$this->load->database();
			$sql = "INSERT INTO block_name(block_title, block_content, category_id, login) 
								VALUES(".$this->db->escape($block_title).",".$this->db->escape($block_content).",
												".$this->db->escape($category_id).",".$this->db->escape($login).")";
			$query = $this->db->query($sql);	
		}
